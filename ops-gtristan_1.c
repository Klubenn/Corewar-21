...
// void	bias_transformer_cutting(int bias, t_player_process *player_process)
// {
// 	if (player_process->flag == true)
// 	{
// 		if (bias >= 0)
// 			player_process->arg_position = (player_process->PC + (bias % IDX_MOD)) % MEM_SIZE;
// 		else if ((-1 * bias) % IDX_MOD <= player_process->PC)
// 			player_process->arg_position = player_process->PC - (bias % IDX_MOD);
// 		else
// 			player_process->arg_position = MEM_SIZE + (player_process->PC + bias % IDX_MOD);
// 	}
// 	else
// 	{
// 		if (bias >= 0)
// 			player_process->arg_position = (player_process->PC + bias) % MEM_SIZE;
// 		else if ((-1 * bias) <= player_process->PC)
// 			player_process->arg_position = player_process->PC - bias;
// 		else
// 			player_process->arg_position = MEM_SIZE + ((player_process->PC + bias) % MEM_SIZE);
// 	}
// }
			...
			t_player_process *player_process, int size, u_int8_t type)//todo previous name put_value_to_arg
{
	...
	if (type == IND_CODE)
	{
		...
		// bias_transformer_cutting((int)number, player_process);
			 ...
			 t_player_list *player_list, t_vm_field_memory *vm_field_memory)
{
	...
	player_process->arg_position = (player_process->PC + 1) % MEM_SIZE;
	arg = (int32_t)take_value_from_field(vm_field_memory, player_process,
		(int)(game_process->op_tab[player_process->operation_code].dir_size), DIR_CODE);
	while (player_list)
	{
		if (arg == (-1 * player_list->position))
			player_list->player->last_live_cycle_number = game_process->cycle_number;
			 ...
			 t_player_list *player_list, t_vm_field_memory *vm_field_memory)
{
	...
	if (player_process->args[1] == REG_CODE)
	{
		...
		put_value_to_field(arg_value[0], vm_field_memory,
			(player_process->PC + (arg_value[1] % IDX_MOD)) % MEM_SIZE);
	move_pc(game_process->op_tab, player_process);
			 ...
			 t_player_list *player_list, t_vm_field_memory *vm_field_memory)
{
	...
	if (player_process->carry == true)
	{
		...
		player_process->PC = (player_process->PC + (arg_value % IDX_MOD)) % MEM_SIZE;
	}
	else
		move_pc(game_process->op_tab, player_process);
}
			...
			t_player_list *player_list, t_vm_field_memory *vm_field_memory)
{
	...
	if ((int)arg_value[2] >= 1 && (int)arg_value[2] <= REG_NUMBER)
	{
		player_process->arg_position = (player_process->PC + arg_value[0] + arg_value[1]) % MEM_SIZE;
		// take_value_from_field();
		put_value_to_register(&(player_process->registers[REG_SIZE * (arg_value[2] - 1)]),
			take_value_from_field(vm_field_memory, player_process, 2, DIR_CODE));
